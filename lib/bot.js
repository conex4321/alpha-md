    ...
    try {
      ...
      conn.ev.on("connection.update", async (s) => {
        ...
        if (connection === "connecting") {
          const { ban, plugins, shutoff} = await personalDB(["ban", "plugins", "shutoff"], { content: {} }, "get");
        ...
        if (connection === "open") {
          ...
          for (const p in plugins) {
            ...
            try {
              const { data } = await axios.get(plugins[p] + "/raw");
              fs.writeFileSync(path.join(__basedir, "plugins", `${p}.js`), data);
              ext_plugins += 1;
              require(path.join(__basedir, "plugins", `${p}.js`));
            } catch (e) {
              ext_plugins = 1;
              await personalDB(["plugins"], { content: { id: p } }, "delete");
              console.log(`Error installing plugin '${p}':`, e);
            }
          }

          fs.readdirSync(path.join(__basedir, "plugins")).forEach((plugin) => {
            if (path.extname(plugin).toLowerCase() === ".js") {
              try {
                require(path.join(__basedir, "plugins", plugin));
              } catch (e) {
                console.log(`Error loading plugin '${plugin}':`, e);
                fs.unlinkSync(path.join(__basedir, "plugins", plugin));
              }
            }
          });
          ...
          const str = `\`\`\`connected sucessfully!\nVersion: ${packageVersion}\nLatestVersion: ${lavss}\nExternal Plugin(s): ${ext_plugins}\nTotal Plugin(s): ${totalPlugins}\nWorktype: ${workType}\`\`\``;      
      ...
      conn.ws.on("CB:call", async (json) => {
        if (json.content[0].tag == "offer") {
          ...
          if (config.CALL_BLOCK) {
            ...
            await conn.sendMessage(callfrom, { text: blockMessage }).catch((e) => console.log(e));
            await conn.updateBlockStatus(callfrom, "block").catch((e) => console.log(e));
          } else if (config.REJECT_CALL) {
            await conn.rejectCall(call_id, callfrom).catch((e) => console.log(e));
            await conn.sendMessage(callfrom, { text: rejectMessage }).catch((e) => console.log(e));
          }
        }
      });
      ...
      conn.ev.on("messages.upsert", async (m) => {
        ...
        plugins.commands.map(async (command) => {
          if (config.DISABLE_PM && !msg.isGroup) return;
          if (config.DISABLE_GRP && msg.isGroup) return;
