...
const GroupDBB = config.DATABASE.define("GroupSnapshot", {
  chat: {
...
async function getSnapshot(jid = null) {
  ...
  const snapshot = await GroupDBB.findAll({
    where: { chat: jid },
  });
...
async function setSnapshot(jid = null, pfp = null, metadata = null) {
  ...
  let existingSnapshot = await GroupDBB.findOne({
    where: { chat: jid },
  });
...
async function deleteSnapshot(jid = null) {
  ...
  const existingSnapshot = await GroupDBB.findOne({
    where: { chat: jid },
  });
