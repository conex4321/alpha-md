...
const { generateWAMessageFromContent, generateWAMessage, WA_DEFAULT_EPHEMERAL } = require("baileys");
...
class AllMessage extends Base {
  ...
  async sendvv(jid, content, options = {}, message) {
    const { data } = await this.client.getFile(content);
    const type = (await fileType.fromBuffer(data)) || {};
  ...
  async reply(text, opt = {}) {
    return this.client.sendMessage(this.jid, { text, ...opt }, { quoted: this.message, ...opt });
  }
